...
from face_smoothing.detector import detect, smooth
...
def process_image(input_img, cfg, detector):
    if input_img is None:
        return None, None, None, None, None, None, None
    # Make sure image is less than 1081px wide
    input_img = check_img_size(input_img)
    # Detect face
    detected_img, faces = detect.detect_face(cfg, detector, input_img)
    bboxes = [face["bbox"] for face in faces]
    output_img, roi_img, hsv_mask, smoothed_roi = smooth.smooth_face(
        cfg,
        input_img,
        faces,
    )
...
def save_steps(filename, all_img_steps, output_height):
    ...
    """
    # Filter out None images and resize
    resized_imgs = tuple(
        resize_image(img, None, output_height) for img in all_img_steps if img is not None
    )
    if not resized_imgs:
        return False
...
def draw_bboxes(output_img, cfg, bboxes):
    ...
    for i in range(len(bboxes)):
        ...
        btm_right = (int(bbox[2]), int(bbox[3]))
        cv2.rectangle(
            output_w_bboxes, top_left, btm_right, cfg["image"]["bbox_color"], 2
        )
