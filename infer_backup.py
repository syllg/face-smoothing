...
def resolve_output_paths(args, cfg):
    output_arg = args.output
    if is_image(output_arg):
        out_dir = os.path.dirname(output_arg) or '.'
        out_base = output_arg
        steps_base = os.path.join(out_dir, cfg['image']['output_steps'])
        video_dir = out_dir
        return out_base, steps_base, video_dir

    out_dir = output_arg
    out_base = os.path.join(out_dir, cfg['image']['output'])
    steps_base = os.path.join(out_dir, cfg['image']['output_steps'])
    video_dir = out_dir
    return out_base, steps_base, video_dir
...
def main(args):
    ...
    cfg = load_configs()
    out_base, steps_base, video_dir = resolve_output_paths(args, cfg)
    # Load the network
